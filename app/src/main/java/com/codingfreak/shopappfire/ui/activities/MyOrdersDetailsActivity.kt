...
class MyOrdersDetailsActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var ordersDetails : Order = Order()
        if(intent.hasExtra(Constants.EXTRA_MY_ORDERS_DETAILS)) {
            ordersDetails = intent.getParcelableExtra<Order>(Constants.EXTRA_MY_ORDERS_DETAILS)!!
        }

        setupUI(ordersDetails)
    }
    ...
    private fun setupUI(orderDetails: Order) {
        ...
        val cartListAdapter =
            CartItemListAdapter(this@MyOrdersDetailsActivity, orderDetails.items, false)
